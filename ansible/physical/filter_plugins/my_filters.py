...
class FilterModule():  # pylint:disable=too-few-public-methods
    ...
    def filters():
        ...
        return {
            'set_chained_crt': set_chained_crt,
            'set_escaped_paths': set_escaped_paths,
            'systemd_escape': systemd_escape,
        }


def set_chained_crt(nginx_ssl_files):
    """Insert a chained certificate into the given dict.
    ...
    place.
    """
    nginx_ssl_files['chained_crt'] = (
        nginx_ssl_files['crt'] + GANDI_STANDARD_SSL_CA_2_PEM
    )
    return nginx_ssl_files
